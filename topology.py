...
class NetworkSlicingTopo(Topo):
    def __init__(self):
        ...
        other_link_config = dict(bw=1)
        video_link_config = dict(bw=100)
        ftp_link_config = dict(bw=500)
        host_link_config = dict(bw=1000)
        ...
        # Add switch links
        self.addLink("s1", "s2", **other_link_config)
        self.addLink("s2", "s4", **other_link_config)
        self.addLink("s1", "s3", **video_link_config)
        self.addLink("s3", "s4", **video_link_config)
        self.addLink("s1", "s5", **ftp_link_config)
        self.addLink("s5", "s4", **ftp_link_config)
